  ...
  <transition name="fade">
    <div class="shelf-title-wrapper" :class="{ 'hide-shadow': ifHideShadow }">
      <div class="title">
        ...
        <span class="sub-title-text" v-show="isEditMode">{{
          selectedText
        }}</span>
      </div>
      <div
        class="btn-text-wrapper"
        @click="onEditClick"
        v-if="!ifGroupEmpty && !isDataEmpty"
      >
        <span class="btn-text">{{
...
export default {
  ...
  computed: {
    isDataEmpty() {
      return (
        !this.data || this.data.filter((item) => item.type !== 3).length === 0
      )
    },
    selectedText() {
